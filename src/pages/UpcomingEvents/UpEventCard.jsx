...
const UpEventCard = ({ events }) => {
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5'>
            {events.map((event, index) => (
                <motion.div key={event._id} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6, delay: index * 0.1 }} className='shadow-sm dark:shadow-md rounded-lg bg-white dark:bg-gray-800'>
