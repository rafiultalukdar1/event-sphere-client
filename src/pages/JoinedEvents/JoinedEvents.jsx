    ...
    <>
        <div className='py-[50px] md:py-[70px]'>
            <div className='container'>
                <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6 }} className='text-center text-[32px] sm:text-[40px] lg:text-[50px] font-bold'>Joined <span className='bg-linear-to-b from-[#219E64] to-[#5FD68E] bg-clip-text text-transparent'>Events</span></motion.h2>
                {joinedEvents.length === 0 ? (
                    ...
                    {joinedEvents.map(event => (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6 }} key={event._id} className='shadow-sm dark:shadow-md rounded-lg bg-white dark:bg-gray-800'>
                            ...
                            <div className='pt-3 px-4 pb-4'>
                                ...
                                <div className="grid grid-cols-2 gap-2.5 mt-4">
                                    {new Date(event.event_date) < new Date() ? (
                                        <button className="py-1.5 bg-red-500 rounded text-white text-[17px] font-medium cursor-not-allowed w-full">Expired</button>
                                    ) : (
                                        <Link to={`/event-details/${event.eventId}`} className="py-1.5 bg-[#219E64] rounded text-white text-[17px] font-medium text-center w-full">View Event</Link>
                                    )}

                                    <button onClick={() => handleDeleteJoinEvent(event.joinedId)} className="py-1.5 bg-[#F43098] rounded w-full text-white text-[17px] font-medium">Close</button>
                                </div>
                            </div>
                        </motion.div>))}
                    </div>
